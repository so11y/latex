...
export function formatterError(v: TemplateStringsArray, ...arg: any[]) {
  const [message, ...teml] = arg
  return teml.reduce((prev, next, index) => {
...
export const ErrorMessage = {
  ...
  ConditionalExpression: {
    ...
    NotNestConditionalExpression: "不支持在直接嵌套Conditional函数和三元表达式,但是可以在搭配逻辑表达式使用",
  },
  Expression: {
